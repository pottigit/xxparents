...
from schoolstudents.MakePublicSign import *
from schoolstudents.ReadDBConfig import *
...
#创建插入token到mongoDB方法
def setToken(file_path,tokendb_index,token_data,loginNo_data,userId_data,setToken_time,timestr):

    try:
        dbserver, dbport, dbdatabase, dbtable = ReadMongo(file_path, tokendb_index).getMongoData()
        conn = ConnectMongoDB(dbserver, dbport, dbdatabase, dbtable)
        conn.runConn()
        token_json['_id'] = timestr
        token_json['ptoken'] = token_data
        token_json['userId'] = str(userId_data)
        token_json['loginNo'] = loginNo_data
        token_json['time'] = setToken_time
...
#创建清空mongoDB中token方法
def clearToken(file_path,tokendb_index):
...
def getToken(file_path,info_index,tokendb_index,user_type):

    #The first time must run clear tokenDB
    #clearToken(file_path,tokendb_index)

    try:
        dbserver, dbport, dbdatabase, dbtable = ReadMongo(file_path, info_index).getMongoData()
        conn = ConnectMongoDB(dbserver, dbport, dbdatabase, dbtable)
        conn.runConn()
        ListData = conn.findOneData({"userType": user_type})
        print "---------------------------------"
        print ListData["loginNo"]
        print "---------------------------------"
        login_no = ListData["loginNo"]
        pass_word = ListData["password"]
        #ParentsLogin().testRecord.info("clear old token successfully")
    except conn, e:
        print "DB error", e

    #http request element(header,body,url)
    headers = {
    ...
    http_params = {
                  "password": hashlib.md5(pass_word).hexdigest(),
                  "loginNo": login_no}
    ...
    if responser['status'] == 200:
        ...
        userId = responser['data']['user']['userId']
        if token:
            set_time = time.strftime("%Y-%m-%d %X", time.localtime())
            time_str = time.time()
            setToken(file_path, tokendb_index, token, userId, login_no, set_time , time_str)
