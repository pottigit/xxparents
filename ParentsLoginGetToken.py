...
class ParentsLogin(object):
    ...
    def __init__(self,login_url,login_data,login_header):
        ...
        self.testRecord = logging.getLogger("QARecord")
    #初始化日志记录方法
    def init_logging(self):
        self.testRecord = logging.getLogger("QARecord")
        filelog = RotatingFileHandler("QARecord.log", maxBytes=20 * 1024 * 1024, backupCount=20)
...
def getToken(file_path,file_index):
    ...
    if responser['status'] == 200:
        #调用解密类DecodeJson对象的decodeData方法，返回token正常字符串值
        #return_str = DecodeJson(responser['data']).decodeData()
        #去掉解密字符串后6位补全字符
        #tokenJson = return_str[:-6]
        #token = simplejson.loads(tokenJson)['token']
        token = responser['data']['token']
        if token:
            setToken(file_path, file_index, token)
            loginer.testRecord.info("get token successfully on ", time.localtime())
        else:
