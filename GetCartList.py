...
class TestCartList():
    ...
    def setup(self):
        ...
        self.dbtable = "cart_list"
        self.payload = {}

        api_url = "http://192.168.0.215:8084/api/shopping/cart/list/userid/%s?"
    ...
    def testGetOneDataFromCartList(self):
        """
        get one data from cart list
        :return:
        """
        try:
            ...
            self.testRecord.info(ListData)
            # 获得基础请求类对象
            httper = HttpBase('GET', self.url % (ListData["userId"]), self.payload, self.token)
            # 使用GET方法请求校区列表接口
            status_code, res = httper.httpRequest()

            if res['status'] == ListData["response"] and len(res['data']['list']) == ListData["cartDataNum"]:
                self.testRecord.info(res['data']['list'])
                assert True
    ...
    def testGetMoreDataFromCartList(self):
        """
        get More data from cart list
        :return:
        """
        try:
            self.conn = ConnectMongoDB(self.dbserver, self.dbport, self.dbdatabase, self.dbtable)
    ...
    def testGetCartListNoUserid(self):
        """
        Not input userid and get data from cart
        :return:
        """
        try:
            self.conn = ConnectMongoDB(self.dbserver, self.dbport, self.dbdatabase, self.dbtable)
